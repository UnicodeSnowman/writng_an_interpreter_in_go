...
// passing file name and io.Reader (pg. 17)

type Lexer struct {
	input        string
	position     int  // current position in input (current char)
	readPosition int  // current next reading position (after current char)
...
func (l *Lexer) readChar() {
	if l.readPosition >= len(l.input) {
		...
		// NOTE won't work for non-ASCII
		// would also need to use `rune` instead
		// of `byte` or Lexer.ch
